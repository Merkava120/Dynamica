...
def canTriggerPotatoKing(argsList):

	kTriggeredData = argsList[0]
	player = gc.getPlayer(kTriggeredData.ePlayer)

	if player.getNumAvailableBonuses(CvUtil.findInfoTypeNum(gc.getBonusInfo,gc.getNumBonusInfos(),'BONUS_POTATO')) > 0:
		return false

	return true


def doPotatoKing2(argsList):
#	Need this because farms are not normally allowed unless there is already farm.
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
def canTriggerTheSeaPeoples(argsList):
...
def getHelpTheSeaPeoples1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	szHelp = localText.getText("TXT_KEY_EVENT_THE_SEA_PEOPLES_HELP_1", ())

	return szHelp


def applyTheSeaPeoples1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	player = gc.getPlayer(kTriggeredData.ePlayer)
	...
	plot = map.plotByIndex(listPlots[gc.getGame().getSorenRandNum(len(listPlots), "Sea Peoples event location")])
...
#		iNumUnits  = 6
	...
	iUnitType1 = CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), 'UNIT_GALLEY')
	iUnitType2 = CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), 'UNIT_SPEARMAN')
	iUnitType3 = CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), 'UNIT_AXEMAN')

	barbPlayer = gc.getPlayer(gc.getBARBARIAN_PLAYER())
	for i in range(1+iNumUnits/2):
		...
		barbPlayer.initUnit(iUnitType3, plot.getX(), plot.getY(), UnitAITypes.UNITAI_ATTACK_CITY_LEMMING, DirectionTypes.DIRECTION_SOUTH)
